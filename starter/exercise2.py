...
class Cart:
    ...
    def total(self) -> float:
        ...
        initialTotal = sum(line["price"]*line["qty"] for line in self.lines)
        return round(initialTotal,2) 

    def __repr__(self) -> str:
        # TODO
        total_qty = sum(line["qty"] for line in self.lines)
        return f"<Cart {total_qty} items, ${self.total():.2f}>"
